...
class WSSC_Mobile_Selector {
    ...
    public function upload_image() {
        ...
        if (move_uploaded_file($file['tmp_name'], $file_path)) {
            ...
            if ($result !== false) {
                ...
                if (!$check_record) {
                    wp_send_json_error('Database record was not created properly');
                }
        ...
        jQuery(document).ready(function($) {
            ...
            imageUpload.on('change', function() {
                ...
                if (file) {
                    ...
                    formData.append('mobile_brand', ''); // No mobile data for image-only shortcode
                    formData.append('mobile_model', ''); // No mobile data for image-only shortcode
    ...
    public function add_cart_item_data($cart_item_data, $product_id, $variation_id) {
        ...
        // Attach image id whether or not brand/model were provided
        if (!empty($image_id)) {
            $cart_item_data['wssc_image_id'] = $image_id;
        }
        ...
        if (!empty($mobile_brand) || !empty($mobile_model) || !empty($image_id)) {
    ...
    public function save_order_item_meta($item, $cart_item_key, $values, $order) {
        ...
        if (!empty($values['wssc_image_id'])) {
            ...
            // Debug logging
            error_log('WSSC Order Debug - Image ID: ' . $values['wssc_image_id']);
            error_log('WSSC Order Debug - Order ID: ' . $order->get_id());
            
            // First check if the image record exists
            $existing_image = $wpdb->get_row($wpdb->prepare(
                "SELECT * FROM $table WHERE id = %d",
                $values['wssc_image_id']
            ));
            
            if ($existing_image) {
                error_log('WSSC Order Debug - Found image record: ' . print_r($existing_image, true));
                
                $result = $wpdb->update(
                    $table,
                    ['order_id' => $order->get_id()],
                    ['id' => $values['wssc_image_id']],
                    ['%d'],
                    ['%d']
                );
                
                if ($result !== false) {
                    error_log('WSSC Order Debug - Successfully updated image with order ID');
                    ...
                    error_log('WSSC Order Debug - Updated record: ' . print_r($updated_image, true));
                } else {
                    error_log('WSSC Order Debug - Failed to update image order_id. DB Error: ' . $wpdb->last_error);
                }
            } else {
                error_log('WSSC Order Debug - Image record not found for ID: ' . $values['wssc_image_id']);
            }
        }
    }
